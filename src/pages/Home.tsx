    ...
    const handleScroll = () => {
      if (parallaxRef.current && contentRef.current && faqRef.current) {
        ...
        if (relativeScroll >= 0 && relativeScroll <= maxParallaxScroll) {
          parallaxRef.current.style.transform = `translateY(${relativeScroll * 0.50}px)`;
        }
        
        // Fix the background position after reaching FAQ section
        if (relativeScroll > maxParallaxScroll) {
          parallaxRef.current.style.transform = `translateY(${maxParallaxScroll * 0.50}px)`;
        }
      }
    };
  ...
  return (
    <>
      ...
      <div className="relative overflow-hidden">
        ...
        <div 
          ref={parallaxRef}
          className="absolute inset-0 w-full h-[150%] z-0 pointer-events-none"
          style={{
